...
class Penggunaan extends CI_Controller {
		...
		$this->form_validation->set_rules('meter_akhir', 'Meter Akhir', 'trim|required',array('required',"Meter akhir harus diisi"));
			if ($this->form_validation->run() == FALSE) {
				...
				if($this->penggunaan->cek_penggunaan()==false){
					$this->session->set_flashdata('pesaneror', 'Tagihan bulan ini sudah ada</div>');
					redirect(base_url('index.php/penggunaan'),'refresh');
				}
				$proses=$this->penggunaan->masuk_db();
				if($proses){
					$this->session->set_flashdata('pesan', 'Tambah penggunaan Berhasil');
				} else {
					$this->session->set_flashdata('pesaneror', 'Tambah penggunaan gagal');
				}
			}
			redirect(base_url('index.php/penggunaan'),'refresh');
	}
	public function hapus($id_pelanggan,$id_penggunaan='')
	{
		...
		if($proses){
			$this->session->set_flashdata('pesan', 'Menghapus penggunaan Berhasil');
		} else {
			$this->session->set_flashdata('pesaneror', 'Menghapus penggunaan gagal');
		}
